...
namespace vkb
{
    Buffer Buffer::create_staging_buffer(DeviceType& device, DeviceSizeType size, const void* data)
    {
        ...
        builder.with_vma_flags(
               ...
               .with_vma_usage(VMA_MEMORY_USAGE_AUTO); // 假设暂存缓冲区使用自动内存

        Buffer staging_buffer(device, builder);

        if (data != nullptr)
        {
            // 假设 Allocated 基类或 Buffer 类提供了 update 方法来映射和拷贝内存
    ...
    Buffer::Buffer(DeviceType& device,
        ...
        // 使用委托构造函数，将参数转发给 Builder 模式的构造函数
        Buffer(device,
               BufferBuilder(size)
    ...
    Buffer::Buffer(DeviceType& device, const BufferBuilder& builder) :
        ParentType(builder.get_allocation_create_info(), nullptr, &device),
