...
namespace vkb
{
    RenderFrame::RenderFrame(vkb::VulkanDevice &device_, std::unique_ptr<RenderTarget> &&render_target, size_t thread_count)
        : device(device_),
          fence_pool{device},
          ...
          descriptor_sets(thread_count)
    {
        ...
        {
            auto [buffer_pools_it, inserted] = buffer_pools.emplace(usage_it.first, std::vector<std::pair<vkb::BufferPool, vkb::BufferBlock *>>{});
            if (!inserted)
            {
            ...
            for (size_t i = 0; i < thread_count; ++i)
            {
                buffer_pools_it->second.push_back(
                    std::make_pair(vkb::BufferPool{device, BUFFER_POOL_BLOCK_SIZE * 1024 * usage_it.second, usage_it.first}, nullptr));
            }
        }
    }
    ...
    vkb::BufferAllocation RenderFrame::allocate_buffer_impl(VkBufferUsageFlags usage, VkDeviceSize size, size_t thread_index)
    {
        // Find a pool for this usage
        auto buffer_pool_it = buffer_pools.find(usage);
        if (buffer_pool_it == buffer_pools.end())
        {
            // vk::to_string 是 vh::cpp 的一部分，已移除。
            // LOGE("No buffer pool for buffer usage {} ", vk::to_string(usage));
            // TODO: LOG("No buffer pool found for the specified usage flags.");
            return vkb::BufferAllocation{}; // 返回一个空的 Allocation 对象
        }

        assert(thread_index < buffer_pool_it->second.size());
        // 类型已更新为 vkb::BufferPool 和 vkb::BufferBlock
    ...
    std::vector<vkb::CommandPool> &RenderFrame::get_command_pools(const vkb::Queue &queue, vkb::CommandBufferResetMode reset_mode)
    {
        ...
        bool inserted = false;
        std::tie(command_pool_it, inserted) = command_pools.emplace(queue.get_family_index(), std::vector<vkb::CommandPool>{});
        if (!inserted)
        {
        ...
        for (size_t i = 0; i < thread_count; i++)
        {
            command_pool_it->second.emplace_back(device, queue.get_family_index(), reinterpret_cast<vkb::RenderFrame *>(this), i, reset_mode);
        }

        return command_pool_it->second;
    }

    vkb::CommandPool &RenderFrame::get_command_pool(const vkb::Queue &queue, vkb::CommandBufferResetMode reset_mode, size_t thread_index)
    {
        assert(thread_index < thread_count && "Thread index is out of bounds");
    ...
    // get_semaphore_pool 函数简化
    // 不再需要模板和 if constexpr，直接返回成员变量
    vkb::SemaphorePool &RenderFrame::get_semaphore_pool()
    {
        return semaphore_pool;
    }

    // get_semaphore_pool 的 const 重载版本
    const vkb::SemaphorePool &RenderFrame::get_semaphore_pool() const
    {
        return semaphore_pool;
    }

    // request_descriptor_set 函数合并了 _impl 的实现，并全面转向 C-API
    // 返回类型和参数类型都已更新
                                                        ...
                                                        size_t thread_index)
    {
        ...
        auto &descriptor_pool = vkb::request_resource(device, nullptr, descriptor_pools[thread_index], descriptor_set_layout);
        if (descriptor_management_strategy == DescriptorManagementStrategy::StoreInCache)
        {
            ...
            if (update_after_bind)
            {
                auto aggregate_binding_to_update = [&bindings_to_update, &descriptor_set_layout](const auto &infos_map)
                {
                    ...
                    {
                        // 使用 Vulkan C-API 的标志位
                        if (!(descriptor_set_layout.get_layout_binding_flag(binding_index) & VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT))
                        {
            ...
            // 假设 vkb::common::request_resource 和 vkb::DescriptorSet 也已更新为使用 C-API 类型
            auto &descriptor_set =
                vkb::request_resource(device, nullptr, descriptor_sets[thread_index], descriptor_set_layout, descriptor_pool, buffer_infos, image_infos);
        ...
        else
        {
            ...
            // 使用去除 HPP 前缀和 Cpp 后缀的统一类型
            vkb::DescriptorSet descriptor_set{device, descriptor_set_layout, descriptor_pool, buffer_infos, image_infos};
