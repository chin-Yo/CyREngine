...
namespace vkb
{
	...
	protected:
		...
		// 提供对 create_info 的可写访问，以便派生类可以修改它
		CreateInfoType &get_create_info();

	protected:
		VmaAllocationCreateInfo alloc_create_info = {};
		CreateInfoType create_info = {}; // 直接存储 C API 的 CreateInfo
	...
	template <typename BuilderType, typename CreateInfoType>
	inline BuilderBase<BuilderType, CreateInfoType>::BuilderBase(const CreateInfoType &create_info_) : create_info{create_info_}
	{
	...
	{
		create_info.sharingMode = (1 < create_info.queueFamilyIndexCount) ? VK_SHARING_MODE_CONCURRENT : VK_SHARING_MODE_EXCLUSIVE;
		return *static_cast<BuilderType *>(this);
	}
